...
try {
    Write-Host "5. Testing simple-iis-app existence check..." -ForegroundColor Yellow
    $existingPool = Get-WebAppPool -Name "simple-iis-app" -ErrorAction SilentlyContinue
    $existingSite = Get-Website -Name "simple-iis-app" -ErrorAction SilentlyContinue
    
    if ($existingPool) {
        Write-Host "   ℹ️  simple-iis-app application pool already exists" -ForegroundColor Blue
    } else {
        Write-Host "   ℹ️  simple-iis-app application pool does not exist (will be created)" -ForegroundColor Blue
    }
    
    if ($existingSite) {
        Write-Host "   ℹ️  simple-iis-app website already exists" -ForegroundColor Blue
    } else {
        Write-Host "   ℹ️  simple-iis-app website does not exist (will be created)" -ForegroundColor Blue
    }
} catch {
    Write-Host "   ❌ simple-iis-app check failed: $($_.Exception.Message)" -ForegroundColor Red
}
